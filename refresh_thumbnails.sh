#!/usr/bin/env bash
#
# Script pour ré‐encoder chaque JPEG/PNG/WebP dans le dossier XY
# afin de forcer macOS à les reconnaître comme de vraies images.
# Usage : ./refresh_thumbnails.sh [chemin_vers_dossier_XY]
#

XY_ROOT="${1:-XY}"

if [ ! -d "$XY_ROOT" ]; then
  echo "Le dossier '$XY_ROOT' n'existe pas."
  exit 1
fi

find "$XY_ROOT" -type f \( -iname "*.jpg" -o -iname "*.jpeg" -o -iname "*.png" -o -iname "*.webp" \) | while IFS= read -r FILE; do
